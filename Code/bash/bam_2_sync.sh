...
# REQUIRES 5 CHANGES FOR CAGE NUMBER. SEE TODO
# Baron Koylass 2025
# v1.0
...
cd /data3/scratch/bty565/cages/cage_2 # CHANGE AS REQUIRED
mkdir sync
...
# Collect files
files=(S02*.bam_intersect.bam) # CHANGE PREFIX AS REQUIRED

declare -A date_map

for file in "${files[@]}"; do
  # Extract last 4 digits after the initial prefix
  mm_yy=$(echo "$file" | sed -E 's/^S02([0-9]{4}).*/\1/') # CHANGE PREFIX AS REQUIRED
...
cd sync
for x in *.bam ; do samtools view -b -F 1024 "$x" > "$x".filtered.bam ; done
# Index filtered bams
for x in *.filtered.bam ; do samtools index "$x" ; done
# Generate mpileup
samtools mpileup 1.bam.filtered.bam 2.bam.filtered.bam 3.bam.filtered.bam 4.bam.filtered.bam 5.bam.filtered.bam 6.bam.filtered.bam 7.bam.filtered.bam 8.bam.filtered.bam 9.bam.filtered.bam --max-depth 0 --min-BQ 0 --fasta-ref /data/home/bty565/empirical/inputs/GCA_000001215.4_full.fna -o cage_2.mpileup # CHANGE AS REQUIRED
# Convert mpileup to sync
perl /data/home/bty565/empirical/inputs/popoolation2_1201/mpileup2sync.pl --input cage_2.mpileup --min-qual 1 --output cage_1.sync # CHANGE AS REQUIRED

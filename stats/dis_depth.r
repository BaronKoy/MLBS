...
freq <- read_csv('/home/baron/Documents/PhD/ML_balancingselection/reuter_data/pileup/depth/output/pop_1/final_pop1.pileup', #change for specific alignment file
...
line_plot <- ggplot(freq, aes(x = pos, y = depth, color = Generation)) +
  geom_line(size = 0.75) +
  theme_grey() +
  labs(title = 'Distribution depth across focal region - pop_1, gen2-56', y = 'Read_depth(n=96)', x = 'Genomic_position_(Chr_3R)') +
